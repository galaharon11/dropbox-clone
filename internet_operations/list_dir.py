...
def list_directory_by_path(server_path, session_id, control_sock, server_ip, group=''):
    ...
    if group:
        control_sock.send(' '.join(['LIST', server_path, group, 'SESSIONID=' + str(session_id)]))
    else:
        control_sock.send(' '.join(['LIST', server_path, 'SESSIONID=' + str(session_id)]))
    files, dirs = data_sock.recv(1024).split(';;;')
